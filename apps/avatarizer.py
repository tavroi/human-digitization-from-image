...
prefix = f"./results/HDFI/obj/{args.name}"
...
if not osp.exists(f"{prefix}_econ_da.obj") or not osp.exists(f"{prefix}_smpl_da.obj"):
    ...
    # remove hands from HDFI for next registeration
    ...
    # remove over-streched+hand faces from HDFI
    econ_da_body = econ_da.copy()
    edge_before = np.sqrt(
    ...
    # stitch the registered SMPL-X body and floating hands to HDFI
...
# re-compute da-pose rot_mat for HDFI
rot_mat_da = smpl_out_lst[1].vertex_transformation.detach()[0][idx[:, 0]]
...
# ----------------------------------------------------
# use any SMPL-X pose to animate HDFI reconstruction
# ----------------------------------------------------
